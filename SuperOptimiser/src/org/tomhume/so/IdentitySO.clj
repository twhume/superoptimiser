...
(def class-name-root "IdentityTest37")
...
(def eq-tests-filter (test-map [zero-is-zero?]))
...
(filter #(try (passes? eq-tests-filter %) (catch VerifyError e (do println e) false)) (map (fn [code] (get-class (:code code)  (str class-name-root "-" (:seq-num code)) meth-name method-signature))
     (expanded-numbered-opcode-sequence 2))) ; generate all 2-sequence bytecodes
